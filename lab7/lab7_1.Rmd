...
msleep %>% 
  ...
  summarize_all(mean) #mean of all the numeric variables, but need to take out NAs
```
...
msleep %>% 
  filter(bodywt>150) %>% 
  distinct() #gives the count of distinct observations
```
...
msleep %>% 
  filter(bodywt>100) %>% 
  summarize(n_genera=n_distinct(genus)) #verb `summarize()` produces summary statistics for a given variable in a data frame
```
...
msleep %>% 
  filter(bodywt>100) %>% 
  summarize(total=n()) #not looking at UNIQUE counts for each genus
```
...
msleep %>% 
  filter(bodywt>100) %>% 
  distinct(genus) %>% 
  pull(genus) #distinct WITH pull() gives a vector 
```
...
```{r}
msleep %>% 
  count(vore, sort=T) #count of each the vore types
```
...
msleep %>% 
  ...
  geom_col() #use col when x and y, not just for counts
```
...
```{r}
msleep %>% 
  count(vore, order) #the carnivores and the orders - getting the counts for each 
```
