...
The study tests if hunting for a large tusk size has resulted in the decline in tusk size (overall) in African elephants. The scientists conducted research both pre and post population recovery, around the time when elephants' tusks were being hunted for. Variables (data recorded) include tusk length, tusk circumference, and mean basal tusk circumference. 
...
elephants %>% 
  ...
  summarize(mean_f=mean(estimated_age_years),
            median_f=median(estimated_age_years),
            sd_f=sd(estimated_age_years),na.rm=T)
```
...
elephants %>% 
  filter(sex=="m") %>% 
  filter(years_of_sample_collection=="2005-13")  %>% 
  summarize(mean_m=mean(estimated_age_years, na.rm=T),
            median_m=median(estimated_age_years, na.rm=T),
            sd_m=sd(estimated_age_years,na.rm=T))
```
```{r}
elephants %>% 
  filter(sex=="f") %>% 
  filter(years_of_sample_collection=="1966-68")  %>% 
  summarize(mean_f=mean(estimated_age_years,na.rm=T),
            median_f=median(estimated_age_years,na.rm=T),
            sd_f=sd(estimated_age_years,na.rm=T))
```
...
ggplot(data=elephants,
      ...
      labs(title="Tusk Length In 1966-68 and 2005-13")
```
...
ggplot(data=elephants_mature,
       mapping=aes(x=years_of_sample_collection,y=tusk_length_in_cm))+
      geom_boxplot(mapping=aes(fill=sex))+
      labs(title="Tusk Length Overtime for Mature Elephants")
```
