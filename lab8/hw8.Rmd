...
fisheries_clean %>% 
  ...
  arrange(desc(sum_catch))
```
NEI stands for Not elsewhere included meaning some of the fishes that are labled NEI are not recording what species or what categories the type of catch belongs in. 
...
fisheries_clean %>% 
  ...
  filter(period=="2023") %>% 
  group_by(country,common_name,period) %>% 
  summarize(sum_catch=sum(catch)) %>% 
  arrange(desc(sum_catch))
```
...
fisheries_clean %>% 
  ...
  labs(title="# of Catches of Alaska pollock(=Walleye poll.) from 2013-2023")
```

10. Perform one exploratory analysis of your choice. Make sure to clearly state the question you are asking before writing any code.
```{r}
#What is the least caught species in 2023 and make a plot of how fishing of this species has changed over 2013-2023. 
fisheries_clean %>% 
  filter(period=="2023") %>% 
  group_by(common_name) %>% 
  summarize(sum_catch=sum(catch)) %>% 
