...
```{r}
elephants %>% 
  summarize(across(where(is.numeric),#summarize across the dataframe, anywhere there is a numeric
                   ~mean(.x, na.rm=T))) #find the mean across all of them and remove all the NAs
```
...
```{r}
elephants %>% 
  group_by(sex) #shows the number of groups 
```
...
elephants %>% 
  ...
  summarize(across(where(is.numeric),
                   ~mean(.x, na.rm=T))) #separates each categorical variable (sex) instead of using filter to find females and males separately
```
#finds the number of samples (could use inconsistent sampling in analysis)
We can group by more than one variable.
```{r}
elephants %>% 
  group_by(sex,years_of_sample_collection) %>% #groups by year of sample collection and sex
  summarize(mean_estimated_age_years=mean(estimated_age_years, na.rm=T), #finds the mean 
            n=n(),
            .groups='keep') %>% 
  arrange(sex)
```
...
msleep %>% 
  group_by(order) %>% 
  summarize(mean_brainwt=mean(brainwt, na.rm=T)) #NaN means not a number (no brain weights for whales)
```
```{r}
msleep %>% 
  filter(order=="Cetacea") %>% 
  select(order, genus, brainwt) #didn't measure any brains for any of the whales
```
