...
```
Herbi is most represented in the data. 

**4. What is the average length of time that mammals sleeps?**
Each mammal sleeps an average of 10.43 hours. 
...
ggplot(data=msleep, 
    ...
    scale_y_log10()+
  labs(title="Ranges of Body Weights by Vore Type")
```
...
```
There is a negative relationship between body weight and the total sleep time. From the plot, as body weight increases, the amount of total sleep decreases as shown from the line. 
...
ggplot(data=msleep,
      ...
      geom_boxplot(mapping=aes(fill=vore))+
    labs(title="Range of Sleep Times by Vore Type")
```
Insecti tends to sleep the most. 
...
ggplot(data=msleep,
    ...
    geom_point(mapping=aes(color=vore))+
    geom_smooth(method = lm, se=T)+
    labs(title="Brain Weight vs. Body Weight")
```
There is a positive relationship between brain weight and body weight. As the brain weight increases, the body weight also increases. 
...
```{r}
#What is the relationship between the amount of REM sleep and the body weight of species?
ggplot(data=msleep,
       mapping=aes(x=bodywt,y=sleep_rem))+
  geom_point(mapping=aes(color=vore))+
  scale_x_log10()+
  geom_smooth(method = lm, se=T)+
  labs(title="REM Sleep Vs. Body Weight")
```
## Knit and Upload
