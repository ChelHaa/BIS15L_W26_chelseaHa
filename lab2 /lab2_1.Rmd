...
```{r}
z=10 #this will work but it will create problems eventually so use <- (option+subtraction sign/dash)
```
...
```{r}
mean(2,8,2) #need to put numbers into a vector to add them
```
...
```{r}
mean(c(venom_GT, chiron, veyron)) #make sure to use a vector
```
...
```{r}
my_integer <- as.integer(my_numeric) #changing numeric into an integer and giving to integer 
```
...
```{r}
classes_of_data1 <- as.numeric(my_integer)
is.integer(classes_of_data1)
```
...
3. How do you interpret this result? What does this mean about NAs? This means that there is some missing information inside the vector that does not allow us to get the mean of new_vector. This means NAs will prohibit use of any methods.
