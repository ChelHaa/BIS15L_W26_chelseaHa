...
```{r}
gabon %>% 
  # ggplot(mapping=aes(y=land_use,x=transect_id))+
  # geom_col(mapping=aes(fill=land_use),position="dodge")+
  # labs(title="# Transects by Land Use Type")
  count(land_use, name="transect_count") %>%
  ggplot(mapping=aes(x = land_use, y=transect_count, fill = land_use)) +
  geom_col() +
  labs(title = "# Transects by Land Use Type", y="# of Transects", x="Land Use Type")
```
...
gabon %>% 
  ...
  geom_col(mapping=aes(fill=land_use))+
  labs(title="Vegetation Richness by Land Use Type", x="Land Use Type", y="vegetation richness")
```
...
gabon %>% 
  ...
  labs(title="Diversity of all Species by Land Use Type")
```
The average for all species diversity is the greatest in Parks. 
...
```
There is a positive relationship between vegetation richness and mammal species richness. 
...
```{r}
# veg_rich is the number of tree species
# DBH = diameter at Breast Height (use veg_dbh)
gabon %>% 
...
gabon %>% 
  ...
  filter(veg_rich > 15 & rich_mammal_species>10) %>% 
  arrange(veg_rich)
```
