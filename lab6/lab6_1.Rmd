...
```{r}
penguins %>%
  select(body_mass_g,flipper_length_mm) %>% #both continuous variables so use a scatter plot
...
```{r}
penguins %>% 
  distinct() #if there are any duplicate rows it will show us, since there are none, the result is the same 344 by 8 dataframe
```

But if we only look at species, we can see that there are only 3 unique species in the data.
```{r}
penguins %>% 
  distinct(species) #looks across the rows and see which ones are duplicated (3 represented species in the species variable)
```

What if we want to know which islands each species occurs on?
```{r}
penguins %>% 
  distinct(species,island, .keep_all=T) #keep_all will keep all the other variables
```
Adelie penguins are on three different islands, gentoo is only on Biscoe island, and chinstrap is only on Dream island. Again, the distinct is looking through rows. 
...
```{r}
penguins %>% 
  mutate(body_mass_kg=body_mass_g/1000) %>% #this adds a new column to the very end of the dataframe
...
```{r}
penguins %>% 
  mutate(across(ends_with("mm"), ~./10)) %>% # ~ means "use this function" > replaced existing columns with "mm" converting them to cm (doesn't change the names)
  select(species,
...
```{r}
mammals %>% 
  mutate(across(everything(),tolower)) #this changes every numeric to a character, be careful with numeric variables (this changes their class)
```
...
mammals <- mammals %>% 
  mutate(across(c(mass,
                  ...
                  afr,
                  litters_year),
                ~ifelse(.==-999,NA,.))) #the period enables the ifelse to go through each cell, looking for -999
```
...
```
Balaenoptera musculus has the highest mass. 
