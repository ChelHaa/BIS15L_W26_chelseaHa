...
homerange %>% 
  ggplot(aes(x=taxon, y=log10_mass, fill=thermoregulation))+
  geom_boxplot()+
  facet_grid(trophic_guild~thermoregulation,labeller=label_both) +#organizes rows to columns so specify how you wanna arrange things
  theme(axis.text.x=element_text(angle=60, hjust=1), #(use .~ and ~.)
        legend.position ="none",
...
homerange %>% 
  ggplot(aes(x=taxon, y=log10_mass, fill=thermoregulation))+
  geom_boxplot()+
  facet_grid(.~thermoregulation,labeller=label_both) +#organizes rows to columns so specify how you wanna arrange things
  theme(axis.text.x=element_text(angle=60, hjust=1), #(use .~ and ~.)
        legend.position ="none",
        strip.text=element_text(size=8, face="bold"))
```
