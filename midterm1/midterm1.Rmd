...
Link to repository:
https://github.com/ChelHaa/BIS15L_W26_chelseaHa
...
```{r}
glimpse(anolis) #check for factors
```
...
labs(title="# Species Sampled by Island", x="Island Type", y="# of Species Sampled")
```
Sampling is not equal across each island. 
...
labs(title="# Species by Habitat", x="Habitat", y="# of Species sampled")
```
Sampling is not equal across habitat type. 
...
```{r}
ggplot(data=anolis, mapping=aes(x=habitat, y=hindlimb_length_mm))+
  geom_boxplot(mapping=aes(fill=habitat))+
  labs(title="Range of Hindlimb Length by Habitat Type")
```
...
anolis %>% 
  ...
  geom_boxplot(mapping=aes(fill=habitat))+
  labs(title="Ratio of Hindlimb to Body by Habitat Type")
```
Grass-Bush: In the first plot, the grass-bush species had the second lowest range for their hindlimb lengths. In this plot, the ratio of their hindlimbs to their body is the second highest meaning although they might have smaller hindlimbs, relative to their size, grass-bush species have a larger hindlimb. 

Trunk-ground: In the first plot, the trunk-ground species had the largest length for their hindlimbs and in this plot, the ratio of their hindlimbs to their body size is also the largest. I might infer that trunk-ground species have an overall large body size and relatively large hindlimbs as well. The same goes for twig species except they have the smallest hindlimb length and ratio of hindlimbs to body size (overall smaller than the rest of the species in the other habitats). 
...
ggplot(data=anolis, mapping=aes(x=body_length_mm,y=tail_length_mm))+
  ...
  labs(title="Body Length vs. Tail Length")
```
Grass-bush lizards have a higher tail length but shorter body length. The data suggests that as body length increases, the tail length also increases, giving a positive relationship. 
...
```{r}
anolis %>% 
  summarize(mean_trunkground=mean(toepad_lamellae_count))
```

```{r}
anolis %>%
  filter(habitat=="Twig") %>% 
  summarize(mean_twig=mean(toepad_lamellae_count))
```

```{r}
anolis %>%
  filter(habitat=="Grass-bush") %>% 
  summarize(mean_grassbush=mean(toepad_lamellae_count))
```
```{r}
anolis %>%
  filter(habitat=="Trunk-crown") %>% 
  summarize(mean_trunkcrown=mean(toepad_lamellae_count))
```
...
anolis %>% 
  ...
  labs(title="# of Toepad Lamellae for Trunk-Crown Species By Island Type")
```
