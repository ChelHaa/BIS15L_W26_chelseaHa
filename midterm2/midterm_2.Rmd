...
summary(polar_bears)
```
Variable Names: bear_id, capture_date, popn, sex, age_class, mating, denning, lat, long, urea_nitrogen, creatinine. 
...
```{r}
#NAs are represented as "unknown" in the variables mating
polar_bears %>%
  summarize(across(where(is.character), ~sum(.x=="unknown"))) 
```
```{r}
#Replacing "unknown" with NA
polar_bears_no_na <- polar_bears %>% 
...
```{r}
miss_var_summary(polar_bears_no_na)
```
...
polar_bears %>% 
  ...
  labs(title="# of Samples by Sex and Population")
```
```{r}
polar_bears %>% 
  count(sex, popn) 
```
Sampling is not even for both sexes. 
...
polar_bears_years %>%
  ...
  labs(title="# of Polar Bears Sampled by Year and Population Type",y="count")
```

5. (2 points) What are the min, median, mean, and max values for `urea_nitrogen` and `creatinine`?
```{r}
polar_bears %>% 
  summarize(urea_nitrogen_min=min(urea_nitrogen, na.rm=T), 
            urea_nitrogen_median=median(urea_nitrogen, na.rm=T),
            urea_nitrogen_mean=mean(urea_nitrogen, na.rm=T),
            urea_nitrogen_max=max(urea_nitrogen, na.rm=T),
            
            creatinine_min=min(creatinine, na.rm=T), 
            creatinine_median=median(creatinine, na.rm=T),
            creatinine_mean=mean(creatinine, na.rm=T),
            creatinine_max=max(creatinine, na.rm=T))
```
...
fasting %>% 
  ...
  count(year, popn, fasting) %>% 
  ggplot(aes(x=year, y=n, color=fasting, group=fasting))+
  geom_line()+
  geom_point(na.rm=T)+
  facet_wrap(~popn)+
  labs(title= "# Fasting Bears by Year and Population Type", y="Number of Bears")+
  theme(axis.text.x = element_text(angle = 90, hjust = 1))
```
...
```{r}
ggmap(map1)+
  geom_point(data=polar_bears, aes(long, lat, color=popn), size=0.75, alpha=0.8)+
  labs(x="Longitude", 
       y="Latitude",
       title="Locations of bears by Population")
```
...
ui <- fluidPage(
  ...
  titlePanel("Range of un_creat_ratio values"),
...
server <- function(input, output, session) {
  ...
  fasting %>% 
      ggplot(aes(x=.data[[input$x]], y=un_creat_ratio))+
