...
## Working directories
```{r}
getwd() #by default you are in the directory that your first opened
#setwd
```
...
```{r}
library("tidyverse") #run this every time
...
```{r}
my_vector_sequence[my_vector_sequence <= 10] #all the objects in the vector and pulling all the values that are less than or equal to 10
```
...
```{r}
hbirds <- tibble(Sex, Length, Weight) #combine these three vectors into a tiblle-a data table, each vector is its own column (variables)
```
