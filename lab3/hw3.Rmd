...
```{r}
mountain_vector <- c("Mount Everest", "K2", "Kangchenjunga", "Lhoste", "Makalu")
mountain_height_feet_vector <- c(29032, 28251, 28169, 27940, 27838)
mountain_height_meters_vector <- c(8848.86, 8611, 8586, 8516, 8485)
```

**2. Combine these vectors into a data frame called `mountains`.**
```{r}
mountains <- tibble(mountain_vector, mountain_height_feet_vector, mountain_height_meters_vector)
```

**3. What is the mean height of the mountains in feet?**
```{r}
mean(mountains$mountain_height_feet_vector)
```
...
mountains <- tibble(mountain_vector, mountain_height_feet_vector, mountain_height_meters_vector,first_climbed)
```
...
```{r}
summits <- c(12884,800,300,400,500)
```

```{r}
mountains <- tibble(mountain_vector, mountain_height_feet_vector, mountain_height_meters_vector,first_climbed,summits)
```
...
```{r}
fatalities <- c(340,96,60,15,40)
```

```{r}
mountains <- tibble(mountain_vector, mountain_height_feet_vector, mountain_height_meters_vector,first_climbed,summits,fatalities)
```
...
```{r}
fatality_rate <- c(2.6,12,20,3.7,8)
```

```{r}
mountains <- tibble(mountain_vector, mountain_height_feet_vector, mountain_height_meters_vector,first_climbed, summits, fatalities, fatality_rate)
```
