...
```{r}
filter(penguins, body_mass_g %in% c(5250,5500)) #pulls out the body masses that are only 5250 grams or 5550 grams
```
...
#comma replaces the & symbol
```

## Pipes `%>%` 
In order to start combining `select()`, `filter()`, and other functions efficiently, we need to learn pipes. Pipes feed the output from one function into the input of another function.  #shift + command +m for the pipe command on mac
...
penguins %>% #select comes first
  select(species,body_mass_g) %>% 
  filter(species=="Gentoo") %>% 
  #slice_max(body_mass_g,n=1) #pulls out largest body mass
  slice_min(body_mass_g, n=1) #pulls out smallest body mass, don't forget to specify the variable
```
...
fish %>% 
  ...
  filter(lakeid=="AL" | lakeid=="AR") %>% #cannot use & because no fish are both from AL and AR at the same time
