...
## `select()`
The verb `select()` allows you to pull out columns of interest from a dataframe; it does not affect the rows. To do this, just add the names of the columns of interest to the `select()` command. The order in which you add them, will determine the order in which they appear in the output.  #recall the rows are variables, the output of select is a smaller dataframe
...
```{r}
msleep_focused <- select(msleep, order, genus, bodywt) #this creates a new dataframe 
```
