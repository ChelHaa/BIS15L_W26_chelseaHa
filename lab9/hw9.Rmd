...
```{r}
sydney_beaches <- sydney_beaches %>% 
  clean_names()
```
Data is tidy: the sites are variables that each have their own observations in their own rows so it is in a long format. 
...
sydneybeaches_wide <- sydneybeaches_long %>% 
  pivot_wider(names_from = "date",
              values_from = "enterococci_cfu_100ml") #now each date (a variable) has its own column, only appears once
sydneybeaches_wide
```
...
sydneybeaches_long %>% 
  ...
  # filter(site=="Little Bay Beach", year=="2013") %>% 
  # summarize(total=sum(enterococci_cfu_100ml, na.rm =T))
```

7. What is the average `enterococci_cfu_100ml` by year (use group by year) for each beach (use group by site). Think about which data you will use- long or wide. Use the long data cuz its easier to use when each observation is in its own row. 
```{r}
sydneybeaches_long %>% 
  group_by(site, date) %>% 
...
sydneybeaches_long %>% 
  ...
  filter(year == 2013) %>%
  group_by(site) %>%
  summarize(total_cfu = sum(enterococci_cfu_100ml, na.rm = TRUE)) %>%
  slice_max(order_by = total_cfu, n = 1)
```


10. Explore the data! Do one analysis of your choice that includes a minimum of three lines of code.  
```{r}
#What is the mean for each council in 2013
sydneybeaches_long %>% 
  separate(date, into=c("day", "month", "year"), sep="/") %>% 
  filter(year == 2013) %>%
  group_by(site) %>% 
