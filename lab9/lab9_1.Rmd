...
variable names are data (the income)
...
relig_income
```
The income is in a character class. 
...
billboard
```
Week is a variable, songs fall off the charts so they are NA, we want each song to have all weeks displayed in one row (one observation for each week in one row). 
...
2. Why are these data not tidy?  
  We want each day's observations to be in its own row. (make the entire data frame longer)
