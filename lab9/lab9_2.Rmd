...
expenditures %>% 
  ...
  pivot_longer(-expenditure,
               names_to= "year",
               values_to= "amount in billions of dollars")
```
...
heartrate2 %>% 
  separate(patient,into=c("patient","gender"), sep=-1) %>% 
  mutate(patient=str_remove(patient,"_")) #gets rid of the underscore
```
...
tb_data
```
Cases and Population are their own variable, so now u wanna make it wide. 
